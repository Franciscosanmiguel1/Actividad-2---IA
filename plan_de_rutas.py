...
def parse_rules(path):
    ...
    with open(path, 'r', encoding='utf-8') as f:
        for raw in f:
            ...
            # create a tuple literal to safely literal_eval
...
def dijkstra_with_lines(start, goal, graph, closed, transfer_penalty):
    # state: (time_so_far, node, current_line, path_list_of_(node,line_used_to_arrive))
    heap = []
    # start with no line (None)
    heapq.heappush(heap, (0.0, start, None, [(start, None)]))
    visited = dict()  # (node,current_line) -> best_time
